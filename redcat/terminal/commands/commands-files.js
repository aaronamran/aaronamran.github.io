...
class FileCommands {
    ...
    chattr(args) {
        // Change file attributes (immutable, append-only, etc.)
        if (this.fs.currentUid !== 0) {
            return { error: 'chattr: Permission denied. Must be root for most attributes.' };
        }
        
        const flags = this.parseFlags(args, ['R', 'V']);
        
        if (flags.args.length < 2) {
            return { error: 'Usage: chattr [-RV] mode files...\n  Operators: + (add) - (remove) = (set exactly)\n  Attributes: i (immutable) a (append only) A (no atime) c (compressed) d (no dump) e (extent format) j (data journaling) s (secure deletion) u (undeletable)' };
        }
        
        const mode = flags.args[0];
        const files = flags.args.slice(1);
        
        // Validate mode format (+i, -i, =i, etc.)
        if (!/^[+\-=][aAcCdDeijPsStTu]+$/.test(mode)) {
            return { error: `chattr: Invalid mode: ${mode}` };
        }
        ...
        for (const file of files) {
            ...
            if (!node) {
                results.push(`chattr: ${file}: No such file or directory`);
                continue;
            }
            ...
            const op = mode[0];
            const attrs = mode.slice(1);
            
            if (op === '+') {
                // Add attributes
                for (const attr of attrs) {
                    if (!node.attributes.includes(attr)) {
                ...
                // Remove attributes
                for (const attr of attrs) {
                    node.attributes = node.attributes.replace(attr, '');
                }
            } else if (op === '=') {
                // Set exactly
            ...
            if (flags.V) {
                results.push(`Flags of ${file} set as ${node.attributes || '---'}`);
            }
        }
    ...
    lsattr(args) {
        const flags = this.parseFlags(args, ['R', 'a', 'd', 'v']);
        const files = flags.args.length > 0 ? flags.args : ['.'];
        
        const results = [];
        for (const file of files) {
            const node = this.fs.getNode(file);
            if (!node) {
                results.push(`lsattr: ${file}: No such file or directory`);
                continue;
            }
            
            // Show attributes
            const attrs = node.attributes || '---';
            const display = attrs.padEnd(20, '-').slice(0, 20);
            results.push(`${display} ${file}`);
            
            // If directory and -R, recurse
            if (flags.R && node.type === 'directory' && node.children) {
                for (const [name, child] of Object.entries(node.children)) {
                    if (!flags.a && name.startsWith('.')) continue;
                    const childAttrs = child.attributes || '---';
                    const childDisplay = childAttrs.padEnd(20, '-').slice(0, 20);
                    results.push(`${childDisplay} ${file}/${name}`);
                }
            }
        }
    ...
    getfattr(args) {
        const flags = this.parseFlags(args, ['d', 'n', 'm', 'R', 'h']);
        
        if (flags.args.length === 0) {
            return { error: 'Usage: getfattr [-dRnmh] [-n name] file...\n  -d: dump all extended attributes\n  -n name: get named attribute\n  -m pattern: match pattern\n  -R: recursive\n  -h: do not follow symlinks' };
        }
        ...
        for (const file of files) {
            const node = this.fs.getNode(file);
            if (!node) {
                results.push(`getfattr: ${file}: No such file or directory`);
                continue;
            }
            
            // Initialize xattrs if not present
            if (!node.xattrs) {
                node.xattrs = {
            ...
            if (flags.n) {
                // Get specific named attribute
                const nameIndex = args.indexOf('-n');
                const attrName = nameIndex !== -1 ? args[nameIndex + 1] : null;
                if (attrName && node.xattrs[attrName]) {
                    results.push(`${attrName}="${node.xattrs[attrName]}"`);
                }
            } else if (flags.d || flags.m) {
                // Dump all attributes
                for (const [name, value] of Object.entries(node.xattrs)) {
                    results.push(`${name}="${value}"`);
                }
            } else {
                // List attribute names only
                for (const name of Object.keys(node.xattrs)) {
                    results.push(name);
                }
            }
    ...
    setfattr(args) {
        if (this.fs.currentUid !== 0) {
            return { error: 'setfattr: Permission denied for most extended attributes.' };
        }
        
        const flags = this.parseFlags(args, ['n', 'v', 'x', 'h']);
        
        if (flags.args.length === 0) {
            return { error: 'Usage: setfattr [-hx] -n name [-v value] file...\n  -n name: set named attribute\n  -v value: set value\n  -x name: remove attribute\n  -h: do not follow symlinks' };
        }
        
        // Get attribute name and value
        const nameIndex = args.indexOf('-n');
        const valueIndex = args.indexOf('-v');
        const removeIndex = args.indexOf('-x');
        
        if (nameIndex === -1 && removeIndex === -1) {
            return { error: 'setfattr: must specify -n name or -x name' };
        }
        
        const attrName = nameIndex !== -1 ? args[nameIndex + 1] : args[removeIndex + 1];
        const attrValue = valueIndex !== -1 ? args[valueIndex + 1] : '';
        ...
        for (const file of files) {
            const node = this.fs.getNode(file);
            if (!node) {
                results.push(`setfattr: ${file}: No such file or directory`);
                continue;
            }
            ...
            if (removeIndex !== -1 || flags.x) {
                // Remove attribute
                delete node.xattrs[attrName];
            } else {
                // Set attribute
                node.xattrs[attrName] = attrValue;
            }
        }
